...
local CMsgLord = class("CMsgLord", require("common.app.CMsgGame"))
...
function CMsgLord:ctor()
    ...
    self.func_game_ = {
        ...
        [G_C_CMD.MDM_GF_GAME] = {   --200
            ...
            [G_C_CMD.SUB_S_Card_Left_Info_Notify] = { func = self.process_200_114, log = "剩余牌数",  debug = true, },
            [G_C_CMD.SUB_S_ALL_USER_CARD_DATA]    = { func = self.process_200_115, log = "全部牌数",  debug = true, },
        },
    }
end
...
function CMsgLord:process_100_100(__data) 
    ...
    local msg = {}
    msg.cbGameStatus     = _buffer:readChar()   --//游戏状态:0/100/101/没有102
    msg.cbAllowLookon     = _buffer:readChar()  --//旁观标志

    LordDataMgr:getInstance():setGameStatus(msg.cbGameStatus)

    --print("process_100_100", msg.cbGameStatus, PlayerInfo.getInstance():getChairID())
...
function CMsgLord:process_100_101(__data)
    ...
    FloatMessage.getInstance():pushMessageDebug("收到跑得快情景消息")
...
function CMsgLord:process_200_103(__data)
    ...
    --保存记牌
    LordDataMgr.getInstance():markLeftCard(msg.wOutCardUser, msg.cbCardData)
...
function CMsgLord:process_200_114(__data)
    ...
    local msg = {}
    for i = 1, 15 do
        msg[i] = _buffer:readUChar()
        print("process_200_114", i, msg[i])
    end

    --处理
    LordDataMgr.getInstance():setLeftCard(msg)
    SLFacade:dispatchCustomEvent(LordScene_Events.MSG_UPDATE_LEFT_CARD)
end

function CMsgLord:process_200_115(__data)
    local _buffer = __data:readData(__data:getReadableSize())
    
    --struct CMD_S_AllUserCardData
    --{
    -- BYTE       cbCardData[3][20]; //扑克数据
    -- BYTE       cbCardCount[3];//玩家手牌数
    --};
    ...
    for i = 0, 2 do
        msg.allCards[i] = {}
        for j = 0, 19 do
            msg.allCards[i][j] = _buffer:readUChar()
        end
    end
    msg.allCounts = {}
    for i = 0, 2 do
        msg.allCounts[i] = _buffer:readUChar()
    end
    SLFacade:dispatchCustomEvent(LordScene_Events.MSG_SHOW_ALL_CARD, msg)
end
...
--剩余牌
function CMsgLord:sendLeftCard()
    local wb = WWBuffer:create()

    self:sendData(G_C_CMD.MDM_GF_GAME, G_C_CMD.SUB_C_Get_Card_Left_Info, wb)
end
