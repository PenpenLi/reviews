...
local LordScene_Define  = require("game.lord.scene.LordSceneDefine")
...
function LordDataMgr:initGameScene(__data)
    ...
    if self.m_cbGameStatus == GS_T_FREE then
        --基础积分
        ...
        msg.iTimesLimit = _buffer:readInt()     --倍数封顶
        msg.wServerType = _buffer:readShort()	--房间类型
        msg.cbTimeAddTimes = _buffer:readChar() --加倍时间
        msg.iOverTimesLimt = _buffer:readUInt() --强制托管次数
    ...
    elseif self.m_cbGameStatus == GS_T_CALL then
        --时间信息
        msg.cbTimeOutCard = _buffer:readChar()		--出牌时间
        msg.cbTimeCallScore = _buffer:readChar() 	--叫分时间
        msg.cbTimeStartGame = _buffer:readChar()	--开始时间
        ...
        msg.cbScoreInfo = {}
        for i = 0, GAME_PLAYER_LANDLORD-1 do
            msg.cbScoreInfo[i] = _buffer:readChar()
        end
        --手上扑克
        msg.cbHandCardData = {}
        for i = 0, NORMAL_COUNT-1 do
        ...
        for i = 0, GAME_PLAYER_LANDLORD-1 do
            msg.cbUserTrusteeShip[i] = _buffer:readChar()
        end
        --房间类型
        ...
        --是否正在加倍
        msg.cbIsAddTimes = _buffer:readChar();
        --庄家用户
        msg.wBankerUser = _buffer:readShort();
        --庄家底牌
        msg.cbBankerCardData = {}
        for i = 0, 2 do
            msg.cbBankerCardData[i] = _buffer:readChar()
        end
        --强制托管次数
        ...
        self.iOverTimesLimt = msg.iOverTimesLimt --强制次数
        ...
        self.m_cbScoreInfo = CopyMemory(self.m_cbScoreInfo, msg.cbScoreInfo, 0, 0, GAME_PLAYER_LANDLORD)
        ...
        self.m_cbIsAddTimesStatus = msg.cbIsAddTimes
        --叫分阶段,不设置地主
        --self.m_wBankerUser = msg.wBankerUser
        self.m_cbBankerCard = CopyMemory(self.m_cbBankerCard, msg.cbBankerCardData, 0, 0, LANDLORD_COUNT)
        self.m_cbHandCardData = CopyMemory(self.m_cbHandCardData, msg.cbHandCardData, 0, 0, NORMAL_COUNT)

        LordGameLogic:getInstance():SortCardList(self.m_cbHandCardData, NORMAL_COUNT, ST_ORDER)
        for i = 0, NORMAL_COUNT-1 do
            print("--server card:", i, msg.cbHandCardData[i])
        end

        if self.m_cbIsAddTimesStatus > 0 then --等于=0是叫分阶段,>1是加倍阶段
            self.m_wBankerUser = msg.wBankerUser --加倍阶段已经确定地主
            --local viewChair = self:SwitchViewChairID(self.m_wBankerUser);
            --self.m_cbHandCardCount[viewChair] = MAX_COUNT;
            self.m_cbHandCardCount[self.m_wBankerUser] = MAX_COUNT;
            
            if self.m_wBankerUser == PlayerInfo.getInstance():getChairID() then
                for i = 0, LANDLORD_COUNT - 1 do
                    print("--banker card:", i, msg.cbBankerCardData[i])
                    self.m_cbHandCardData[NORMAL_COUNT + i] = msg.cbBankerCardData[i]
                end
                for i = 0, self.m_cbHandCardCount[self:GetMeChairID()] - 1 do
                    print("--handcard:", self.m_cbHandCardData[i]);
                end
                LordGameLogic:getInstance():SortCardList(self.m_cbHandCardData, MAX_COUNT, ST_ORDER);
                self:searchOutCard();
            end
        end

    elseif self.m_cbGameStatus == GS_T_PLAY then
        --时间信息
        msg.cbTimeOutCard = _buffer:readChar()		--出牌时间
        msg.cbTimeCallScore = _buffer:readChar()	--叫分时间
        msg.cbTimeStartGame = _buffer:readChar()	--开始时间
        ...
        for i = 0, GAME_PLAYER_LANDLORD-1 do
            msg.cbUserOffLineTimes[i] = _buffer:readChar()
        end
        --倍数封顶
        msg.iTimesLimit = _buffer:readInt()
        --玩家托管
...
function LordDataMgr:OnConclude(pGameConclude)
    ...
    for i = 0, 2 do
        self.m_pGameConclude.lGameScore[i] = pGameConclude.lGameScore[i]
    end
    self.m_pGameConclude.bChunTian = pGameConclude.bChunTian
    self.m_pGameConclude.bFanChunTian = pGameConclude.bFanChunTian
    
    self.m_pGameConclude.cbBombCount = pGameConclude.cbBombCount
    ...
    for i = 0, 2 do
        self.m_pGameConclude.cbCardCount[i] = pGameConclude.cbCardCount[i]
    end
    self.m_pGameConclude.cbHandCardData = {}
    for i = 0, 54-1 do
        self.m_pGameConclude.cbHandCardData[i] = pGameConclude.cbHandCardData[i]
    end
...
function LordDataMgr:updateUserGender()
    ...
    for i = 0, 2 do
        ...
        if user.dwUserID > 0 then
            ...
            self.m_cbGender[i] = self:getGenderByFaceID(user.wFaceID)
            print("ID:", user.dwUserID, "椅子：", i, "自己", nChairID, "性别", user.cbGender)
        end
    end
end

function LordDataMgr:getGenderByFaceID(faceID)
    ...
    --faceID[8]gender[1-女]
    --faceID[9]gender[0-男]
    if faceID == 0 or faceID == 1 or faceID == 4 or faceID == 7 or faceID == 8 then
        return 1
    elseif faceID == 2 or faceID == 3 or faceID == 5 or faceID == 6 or faceID == 9 then
        return 0
    end
end
...
function LordDataMgr:getChuntianCount()
    if self.m_pGameConclude.bChunTian and self.m_pGameConclude.bChunTian > 0 then
        return 1
    elseif self.m_pGameConclude.bFanChunTian and self.m_pGameConclude.bFanChunTian > 0 then
        return 1
    end
    return 0
end
