...
CBetManager.eMONEY_ONE   = 1000    --1000
CBetManager.eMONEY_TWO   = 10000   --10000
CBetManager.eMONEY_THREE = 100000   --100000
CBetManager.eMONEY_FOUR  = 1000000  --1000000
CBetManager.eMONEY_FIVE  = 5000000  --5000000
CBetManager.eMONEY_SIX   = 10000000 --10000000
-- 定义 ------------------------------

local JETTON_ITEM_COUNT = 4 -- 6个筹码
...
function CBetManager:ctor()
    ...
    self.m_lPlayerScoreOfCutRevenue = 0 --玩家结果，扣税之后
    ...
    --对应筹码的权值
    self.m_llJettonWeight = {0.01,0.09,0.4,4.5,20,75}
...
function CBetManager:setAllJettonScore(info)
    self.m_llJettonValue = info 
end
...
function CBetManager:getSplitGoldNew(llValue)
    local vecRet = {}
    local tempVecRet = {}
    if llValue < 50000000 then
        tempVecRet = self:getSplitGold(llValue)
    else
        tempVecRet = self:getStaticSplitGold(llValue)
    end

    local table_len = #tempVecRet
    for i=1,table_len do
        local rand_num = math.random(1,#tempVecRet)
        table.insert(vecRet,tempVecRet[rand_num])
        table.remove(tempVecRet,rand_num)
    end

    return vecRet
end

function CBetManager:getStaticSplitGold(llValue)
    local vecRet = {}
    local temp_num_list = {}
    for k,jetton_value in pairs(self.m_llJettonValue) do
        local temp_num = math.floor(llValue*(self.m_llJettonWeight[k]/100)/jetton_value)
        table.insert(temp_num_list,temp_num)
    end
    -- dump(temp_num_list,"拆分筹码个数表"..llValue)

    for i=1,#temp_num_list do
        local temp_len = temp_num_list[i]
        for j=1,temp_len do
            llValue = llValue - self.m_llJettonValue[i]
            table.insert(vecRet,self.m_llJettonValue[i])
        end
    end

    --将剩余值 从最大到小再拆分一次
    local vecRet_Two = self:getSplitGold(llValue)
    for i=1,#vecRet_Two do
        table.insert(vecRet,vecRet_Two[i])
    end
    -- print("----getStaticSplitGold---",#vecRet,#vecRet_Two)
    vecRet_Two = nil
    return vecRet
end
...
function CBetManager:setPlayerScoreOfCutRevenue(score)
    self.m_lPlayerScoreOfCutRevenue = score
end
function CBetManager:getPlayerScoreOfCutRevenue(score)
    return self.m_lPlayerScoreOfCutRevenue
end
