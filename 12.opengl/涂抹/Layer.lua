...
function prototype:touchHit(x, y)
    ...
    local rect = CCRectMake(-ox + self.touchHitOffset.x, -oy + self.touchHitOffset.y, size.width * self.touchHitScaleX, size.height * self.touchHitScaleY)
...
function prototype:onTouch(eventType, x, y)
    ...
    for _, v in ipairs(self.touchResponders) do
        ...
        local isHit = v.touchPriority == MAX_TOUCH_PRIORITY
        if not isHit then
            if eventType == "began" then
                isHit = v:touchHit(x, y)
            end
        end
...
function prototype:reorderTouchResponders()
    table.sort(self.touchResponders, function(a, b) return a.touchPriority > b.touchPriority end)
end
