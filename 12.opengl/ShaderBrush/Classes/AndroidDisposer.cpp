...
//#import  "AndroidDisposerViewController.h"
...
{
#if (CC_TARGET_PLATFORM == CC_PLATFORM_IOS)
//    AndroidDisposerViewController* AndroidDisposerViewController = [[AndroidDisposerViewController alloc] initWithNibName:nil bundle:nil];
//    
//    RootViewController* _viewController = (RootViewController*)m_viewController;
//    [_viewController.view addSubview:AndroidDisposerViewController.view];
//    
//    [AndroidDisposerViewController localPhoto];
#endif
...
//    [_viewController.view addSubview:AndroidDisposerViewController.view];
//    
//    [AndroidDisposerViewController takePhoto];
#endif
...
bool AndroidDisposer::init()
{
 ...
 //   m_layer->addChild(sprite);
	...
	m_pListViewCheckBoxs->setContentSize(Size(sizeCheck.width, 0));
	...
	m_pListViewCheckBoxs->setPosition(Vec2(visibleSize.width / 2, 0));
	...
	auto cancelCallback = [=](Ref*){
		...
		MoveBy* move = MoveBy::create(0.2f, Vec2(0, -height));
		m_pListViewCheckBoxs->runAction(move->clone());
		Sequence* seq = Sequence::createWithTwoActions(FadeOut::create(0.2f), RemoveSelf::create());
	...
	addListItem(m_pListViewCheckBoxs, CONTROL_BUTTONMID, UTEXT("选取现有图片"), [=](Ref*){ openPhoto(); cancelCallback(nullptr); });
	addListItem(m_pListViewCheckBoxs, CONTROL_BUTTONMID, UTEXT("照相"), [=](Ref*){ openCamera(); cancelCallback(nullptr); });
	addListItem(m_pListViewCheckBoxs, CONTROL_TRANSPARENTX, "", [=](Ref*){});
	addListItem(m_pListViewCheckBoxs, CONTROL_BUTTONDOWN, UTEXT("取消"), [=](Ref*){ cancelCallback(nullptr); });
	addListItem(m_pListViewCheckBoxs, CONTROL_TRANSPARENTX, "", [=](Ref*){});
    ...
    Director::getInstance()->getRunningScene()->scheduleOnce([=](float time)
    {
        ...
        MoveBy* move = MoveBy::create(0.3f, Vec2(0, height));
		m_pListViewCheckBoxs->runAction(move->clone());

        m_layer->setOpacity(0);
        m_layer->runAction(FadeTo::create(0.2f, 125));

    }, 0.1f, "AndroidDisposerScheduleOnce");
    //-------------------------------------
    // 截断事件
    //-------------------------------------
	...
	const std::string& strText, const std::function<void(Ref*)>& callback)
{
	...
	pListView->setContentSize(Size(pListView->getContentSize().width, pListView->getContentSize().height + sizeCheck.height));
